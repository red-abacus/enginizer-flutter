...
class CarReceptionFormModal extends StatefulWidget {
  AppointmentDetail appointmentDetail;

  CarReceptionFormModal({this.appointmentDetail});

  @override
  State<StatefulWidget> createState() {
    return _CarReceptionFormModalState();
  }
}

class _CarReceptionFormModalState extends State<CarReceptionFormModal> {
  ...
  Widget _buildContent() {
    return _receiveCarFormState == ReceiveCarFormState.FORM
        ? _formContainer()
        : CarReceptionFormWidget(
            appointmentDetail: widget.appointmentDetail, refreshState: null);
  }
              ...
              child: RichText(
                text: TextSpan(
                    ...
                    children: <TextSpan>[
                      ...
                      TextSpan(
                        text:
                            ' ${widget.appointmentDetail?.user?.name ?? 'Mircea Pop'} ',
                        style: TextHelper.customTextStyle(
                            null, gray3, FontWeight.bold, 16),
                      ),
                      TextSpan(
              ...
              child: RichText(
                text: TextSpan(
                    ...
                    children: <TextSpan>[
                      TextSpan(
                        text:
                            '${widget.appointmentDetail?.car?.registrationNumber ?? 'CJ-12-TPT'}, ${widget.appointmentDetail?.car?.brand?.name ?? 'SEAT IBIZA'}, ${widget.appointmentDetail?.car?.year?.name ?? '1992'}, ${widget.appointmentDetail?.car?.color?.translateColorName(context) ?? 'Albastru'}',
                        style: TextHelper.customTextStyle(
                            null, gray3, FontWeight.bold, 16),
                      ),
                    ]),
